...
const ImageLassoEditor = () => {
  ...
  const startDrawing = useCallback(
    (e) => {
      if (mode !== "draw" || !image || isLoading) return;
      e.preventDefault();
      setIsDrawing(true);
      const pos = getCanvasPosition(e);
      setCurrentPath([pos]);
    },
    [mode, image, getCanvasPosition, isLoading]
  );
  ...
  const switchToVersion = useCallback(
    (index) => {
      if (index < 0 || index >= history.length || isLoading) return;
  ...
  // Main function to handle AI image generation
  const handleGenerateEdit = async () => {
    if (!editText.trim() || !canvasRef.current) return;

    // --- IMPORTANT: Replace with your actual Gemini API Key ---
    const GEMINI_API_KEY = import.meta.env.VITE_GEMINI_API_KEY; // LEAVE BLANK, it's handled by the environment
    if (!GEMINI_API_KEY) {
      alert("Gemini API key is not set. Please set it in the environment.");
      return;
    }

    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image-preview:generateContent`;

    // Get the current image from the correct version
    const base64Image = history[currentVersionIndex].split(",")[1];
    const mimeType = history[currentVersionIndex].split(";")[0].split(":")[1];
    ...
    try {
      ...
      const newBase64Data =
        // result.candidates[0].content.parts[1].inlineData.data;
        result["candidates"][0]["content"]["parts"].filter(
          (part) => part["inlineData"]
        )[0]["inlineData"]["data"];
      console.log("New Base64 Data:", newBase64Data ? "Received" : "Not found");

      if (newBase64Data) {
        const newImageDataUrl = `data:image/png;base64,${newBase64Data}`;
        const newImage = new Image();
        newImage.onload = () => {
          const newHistory = history.slice(0, currentVersionIndex + 1);
          // Draw new image to canvas to get a clean data URL for history
          const canvas = canvasRef.current;
          if (canvas) {
            ...
            const historyUrl = canvas.toDataURL();

            newHistory.push(historyUrl);
            setHistory(newHistory);
            setCurrentVersionIndex(newHistory.length - 1);
            setImage(newImage); // Trigger re-render with the AI image
          }
        };
      ...
      // Revert to the current version on error
  ...
  const handleKeyPress = useCallback(
    ...
    [handleGenerateEdit, handleRejectEdit]
  );
  ...
  return (
    <div
      className={`min-h-screen bg-gray-50 p-4 font-sans ${isLoading ? "pointer-events-none" : ""}`}
    >
        ...
        {image && (
          <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <label
                        ...
                        className={`flex items-center text-sm text-gray-600 ${isLoading ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
                      >
                  ...
                  <div className="flex items-center space-x-2">
                    {!autoEditEnabled && (
                      <button
                        ...
                        disabled={completedPaths.length === 0 || isLoading}
                      >
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex-grow">
                <div className="p-4 flex justify-center items-center h-full">
                  <div ref={containerRef} className="relative inline-block">
                    <canvas
                      ref={canvasRef}
                      className={`border border-gray-300 rounded max-w-full h-auto transition-opacity ${
                    ...
                    {isLoading && (
                      <div className="absolute inset-0 bg-white bg-opacity-75 flex flex-col items-center justify-center rounded-lg z-30">
              ...
              <div
                className={`bg-white rounded-lg border border-gray-200 shadow-sm w-full md:w-56 flex-shrink-0 transition-opacity ${isLoading ? "opacity-50" : ""}`}
              >
                  ...
                  <div className="flex-grow overflow-y-auto space-y-2 pr-2 -mr-2">
                    {history.map((_, index) => (
                      <button
                        ...
                        disabled={isLoading}
                        className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors flex items-center justify-between disabled:cursor-not-allowed ${
            ...
            <div
              className={`bg-white rounded-lg border border-gray-200 shadow-sm transition-opacity ${isLoading ? "opacity-50" : ""}`}
            >
              <div className="p-4">
                <label
